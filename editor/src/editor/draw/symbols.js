(function () {
  ...
  b3e.draw.anchorSymbol = function (block, settings) {
    var shape = new createjs.Shape();
    shape.graphics.beginFill('#AA0000');
    var border_width = settings.get('anchor_border_width');
    shape.graphics.setStrokeStyle(border_width, 'round');
    shape.graphics.beginStroke(settings.get('block_border_color'));
    shape.graphics.drawCircle(settings.get('anchor_offset_x'), settings.get('anchor_offset_x'), 12);
    shape.graphics.endStroke();
    shape.graphics.endFill();
  }
  ...
  b3e.draw.SYMBOLS = {
    ...
    'BreakpointAnchor': b3e.draw.anchorSymbol
  };
