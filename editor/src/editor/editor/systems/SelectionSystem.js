b3e.editor.SelectionSystem = function (editor) {
  ...
  this.onMouseOver = function (e) {
    ...
    var x = point.x;
    var y = point.y;
    blockHovered = tree.blocks.getUnderPoint(x, y);
    setTimeout(() => {
      if (blockHovered) {
        editor.trigger('tooltip', blockHovered);
      }
    }, 1000);
  }
  ...
  this.onMouseOut = function (e) {
    ...
    try {
      editor.trigger('tooltip-off', blockHovered);
    } catch (ex) {
      console.error(ex)
    }
    blockHovered = null;
  ...
  editor._game.stage.on('rollover', this.onMouseOver, this);
  editor._game.stage.on('rollout', this.onMouseOut, this);
};
